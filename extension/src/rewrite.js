...
const digrams = memo(() => [parse_digrams(digrams_fr_raw),
                            parse_digrams(digrams_en_raw)])
...
function probably_en_rather_than_fr(str) {
    const [fr, en] = digrams()
    let fr_entropy = 0;
    let en_entropy = 0;
    for (const word of str.split(/(\p{L}+(?:[-:./_0-9?]+\p{L}+)*)/u)) {
        for (let i = 0; i < word.length - 1; i++) {
            const digram = word[i] + word[i+1];
    ...
    const expl = [en_entropy, fr_entropy]
    return [en_entropy < fr_entropy, expl]
}

// manually included from data/english-freq/erofa_intersection
const erofa_en_intersection_raw = "tell hello matter phone office running million pull ladies happening suppose terrible bill attention village support type innocent roll excellent commander correct challenge photo horrible planning staff style bath attorney official jenny shopping media chatter arrive penny intelligence cigarette effort senior warrant attitude bull commit opposite billion dollar argue collection donna charlotte tunnel teddy gym arrange butter attend attractive pattern boxes comment habit intelligent occasion homicide offense chaos math commercial williams drill matches jazz alpha whiskey bitter granny commission communication satellite effective affect phase whip tennis technique kidnapping bonnie pierre khan cherry halloween alliance humble coffin personnel application penalty psycho villa cycle addition chi arrangement affection connections lobby illusion travelling mademoiselle collapse phrase programme ego phoenix bicycle innocence ballet arrogant offensive buffalo hobby hostile cottage chopper honorable tanner dynamite pudding interrogation belle accuse bizarre betting corridor ciao addict corruption orchestra whisky canyon efficient chorus syndrome shipping spaghetti horizon curry grill marketing occupation hurricane skipper exhibition collector opposition roller attraction ferry sherry tequila taller morphine rabbi stripper carrier caller immune immigration addiction microphone isabelle humour crashes humiliation hotter scooter apparent paddy collective banner sniff ghetto marathon affirmative apocalypse catastrophe chauffeur immense homo sierra cutter scanner buffet philosopher flatter gamma hypocrite attendant oppose collision commence cameraman accusation kidnapper prototype penthouse troll revolver lynch commandant martyr arrogance commotion latte immature correction ripper pennies imminent diesel allie commerce babysitting seller zipper terrain bulletin brunette hippie mammy maths communion sketches gallons immoral pollution lotta offshore scanning thermal alligator graffiti derrick appoint lorry gallant scatter porridge syringe rhino habitat supplier hast alphabet interviewer flashes immigrant incorrect risotto kappa cassette intellect cellular portfolio shah bette volleyball roulette dyke bonnet accord narrative trillion exhale collaboration excellence python commando terri oppression pollen mayonnaise rocker mahjong commune innovation latter correspondent interruption ballot exhaustion hormone horoscope commodore trolley impeccable horizontal techno softball installation barricade gallon syphilis hallucination accommodation lemme stripping orchestral elle caterpillar blizzard hui ping-pong collier zapping harmonica sheikh lotte jacuzzi reconnaissance terra pepperoni cholera vendetta jaffa rapper cannabis donner loch caddy hippo constellation bordeaux attire saxophone prix chao sioux veto baht attentive marshmallow rushes ferret supporter wharf cranberry shilling synchro commute addictive riff omelette affidavit anthrax territorial mannequin corral kebab flutter hospice communal pamphlet goddam quitter gazette synagogue lyric sphinx recycle occurrence jazzy cheddar comma silhouette nylon inhaler pierrot annihilation curriculum dahlia boycott hyper irritable aphrodite hombre cyclone affliction chardonnay regrettable phi reggae chrome corvette inflammation penalties irrigation gimmick krypton piccolo durham barrage yiddish analyse zeppelin hippy lithium confetti gazelle enzyme pedigree correspond torah sophistication delirium dribble trotter referendum agha rapport hormonal allez palette acupuncture gymnasium manoeuvre physique questionnaire attaché shimmy appétit gramophone deux ballast programmer narration oppressive toffee thane mystique photon apparition mozzarella vaudeville terrier polyester piranha ville paulette incorrigible flipper suffocation strapping clipper lobbying typhus faux suppression hydrant paella balthazar ruffian halle hibernation barracuda kohl alla thermostat soufflé dynamo chlamydia symposium shabbat bitte irritation habitable offset insurrection vermouth inhalation allure violette chromosome poly largo pellet absinthe affront diffuse souffle placebo mahatma hacienda mollie pizzeria mette backgammon moloch torrent larynx dauphin collage stimuli ortho allo symbiote haddock collusion cameramen tricycle tories arroyo intermittent gaffer scythe paddock aberration technicolor steppe succulent strychnine hubris sniffer illustration apartheid aggravation typo affluent gnocchi handball siphon lorries mahdi occupant illumination immersion palle lynx matte jeannette parr volley choral baguette phlox hypertension sphincter accumulation guppy hydrate uppercut sonnet commuter baller mille syllabus lyre sombrero scampi affiliation doppler calypso chiffon bonne maharaja nipper errant hydro seppuku mullah hosanna ammonium tonne opportune salmonella hymen attache pelle palladium corrosive exhume affirmation raff vinaigrette menthol supposition sympathise effendi squatter applicable hier fritter linoleum pharma commencement stopper foetus admittance hovercraft agathe peso hiatus cellulite maharajah marquette sylvain cellophane papyrus accuser triathlon exhaustive coeur ricotta baccarat salle calle somme immobile camorra rho eucalyptus milli corroboration skunks philo trekking krill ranches corrective guttural casbah pneumothorax exhumation claymore hypotensive agrippa schizo hyperactive plaît malle terre puffin collette croix jaffe chianti thorax guerre hibiscus tachyon thermite commode banyan julienne kyu grizzlies ayatollah aux graphite skiff credo gibbon daimyo hara-kiri phallus inexhaustible lisette hetman communiqué artefact ferrer serra maître pathos potpourri caddie honore kitsch impresario roux collaborative rottweiler rosette pitbull benoît rajah slashes allocation flippant churros corrosion ferries chinchilla vanna incorruptible nympho abattoir habitation approximation grappa edelweiss photocopier trimmer hot-dog statuette physio homophobe favela oxymoron scull thora whist pentothal schilling donne butte desperado camellia apostrophe barra calla narra hypothalamus berthe hermaphrodite valkyries suffragette whiskies clapper bodhisattva panna tortellini buller staffer pirouette addendum atoll attrition hypo lycra coccyx sommelier setter quelle thallium manette aberrant suffrage harmonium honoré onyx week-end lichen balle asphyxia additive orchestration lemming colley allegro menthe affable sommer hidalgo synopsis morphing opposable harakiri excommunication pyjama phosphate mitochondrial hippodrome ahana hyperbole tulle appropriation gyroscope effusion inhibition wah-wah coefficient hadron inopportune paraphrase dilettante yucca villette catharsis xylophone communique"
...
function collides_with_english(cache, norm_word) {
    // Observed bad characterisations as english:
    // "Comment stocker des fichiers sur de l'ADN" (because of "ck")
    // "Comment Taiwan domine le monde" (because of "wa")
    // "Support pour d'autres orthographes" (not sure why either)
    // "Formats photos" (quite difficult, as both words are english and french)
    //
    // In the other direction:
    // "an alien intelligence"
    //
    // This might be improvable by a combination of:
    // - having digrams that includes apostrophes, as d' or l' are very
    ...
    //
    // Also it's not clear why we split the text in words, rather than processing
    ...
    if (cache.text_is_en != null) {
        ...
        cache.text_is_en =
            ...
            ? [true, 'idempotence']
            : probably_en_rather_than_fr(cache.node.nodeValue);
...
function rewrite_under(options, table, root){
    ...
    while(n=walk.nextNode()) {
        const cache =
              options.fren
              ? { text_is_en: null, node: n, debug: false }
              : null;
        let regular_text = ""
