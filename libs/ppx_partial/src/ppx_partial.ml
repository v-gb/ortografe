...
let bind expr f =
  ...
  | Pexp_ident _ -> f expr
  | _ ->
      pexp_let ~loc:expr.pexp_loc Nonrecursive
        [ value_binding ~loc:expr.pexp_loc
...
let beta_redex ~loc params body exprs = eapply ~loc (eabstract ~loc params body) exprs
...
let with_inferred_type_of_arg ~loc (first_params, last_param) body first_args =
  ...
  then
    beta_redex ~loc first_params
      [%expr
        ();
        fun [%p last_param] -> [%e body]]
      first_args
  else
    [%expr
      let _partial_arg_type =
        (* ('v2 -> unit) option, for a fresh non generalized 'v2.
      ...
      in
      if false
      then (fun (* unify 'v2 with the expected type of v2 *)
                  x ->
        (match _partial_arg_type with None -> () | Some f -> f x);
        assert false)
      else
        [%e
          beta_redex ~loc first_params
...
let rewrite ~loc f params =
  ...
  if willing_to_reexecute f && List.for_all (fun (_, e) -> willing_to_reexecute e) params
  then
    ...
    let replace_placeholder e =
      match e with [%expr __] -> partial_evar ~loc:e.pexp_loc 1 | _ -> e
    in
    let args = List.map (fun (arg, e) -> (arg, replace_placeholder e)) params in
    eabstract ~loc [ partial_pvar ~loc 1 ] (pexp_apply ~loc (replace_placeholder f) args)
  else
    (* Otherwise, generate [(fun p1 p3 p2 -> f p1 p2 p3) e1 e2] (roughly, see
       with_inferred_type_of_arg for the gory details). *)
    bind f (fun f ->
        let remaining_params =
          List.filter_map (function _, [%expr __] -> None | _, e -> Some e) params
        in
        let body =
          pexp_apply ~loc
            (match f with [%expr __] -> partial_evar ~loc:f.pexp_loc (-1) | _ -> f)
            (List.mapi (fun i (arg, e) -> (arg, partial_evar ~loc:e.pexp_loc i)) params)
        in
        ...
        in
        with_inferred_type_of_arg ~loc (first_params, last_param) body remaining_params)
      ...
      (object
         ...
         method! expression e =
           ...
           | { pexp_desc = Pexp_field (([%expr __] as placeholder), fieldname); _ } ->
               ...
               [%expr fun x -> [%e e']]
           | { pexp_desc = Pexp_construct (constructor, Some ([%expr __] as placeholder))
             ; _
             } ->
               let e' =
                 let loc = placeholder.pexp_loc in
               ...
               let loc = e.pexp_loc in
               [%expr fun x -> [%e e']]
           | { pexp_desc = Pexp_variant (constructor, Some ([%expr __] as placeholder))
             ; _
             } ->
               let e' =
                 let loc = placeholder.pexp_loc in
           ...
           | { pexp_desc = Pexp_apply (f, params); _ } ->
               let count =
                 List.fold_left
                   (fun acc (_, e) -> match e with [%expr __] -> acc + 1 | _ -> acc)
                   (match f with [%expr __] -> 1 | _ -> 0)
                   params
               in
               if count = 0
               then e
               ...
               then
                 let replace_placeholder e =
                   match e with
                   | [%expr __] ->
                       pexp_extension ~loc:e.pexp_loc
                         (Location.error_extensionf ~loc:e.pexp_loc
                            "ppx_partial: only one __ argument is supported per function \
                             call")
                   | e -> e
                 in
                 let f = replace_placeholder f in
                 let params =
                   List.map (fun (arg, e) -> (arg, replace_placeholder e)) params
                 in
