      ...
      | Pexp_apply (_, [(Nolabel, x)]) ->
         (match x.pexp_desc with
          | Pexp_constant (Pconst_string (str, _, _)) ->
             ...
             then Location.raise_errorf ~loc
                    "%s is applied to invalid utf8" fun_;
             ...
             Some [%expr Stdlib.Uchar.unsafe_of_int [%e Ast_builder.Default.eint ~loc (Stdlib.Uchar.to_int uchar)]]
          | _ -> Location.raise_errorf ~loc
                   "%s must be applied to a string literal" fun_)
      | _ -> Location.raise_errorf ~loc
               "%s must be applied to a string literal" fun_)

let () =
  Driver.register_transformation "ppx_uchar_literal"
    ~rules:[ rule ]
;;
