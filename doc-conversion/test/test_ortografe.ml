...
let pp_xml src =
  Markup.parse_xml (Markup.string src)
  |> Markup.signals
  |> Markup.pretty_print
  |> Markup.write_xml
  |> Markup.to_string
...
"
  ...
  let rewrite3 =
    Ortografe.pure_text ~dst:String
      ~options:
        { options with
          dict = Stdlib.Hashtbl.find_opt (force Ortografe_embedded.rect1990)
        }
      {|
...
let docx_document zip = Ortografe.Private.grab_from_zip zip "word/document.xml"
...
<w:document xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:w10="urn:schemas-microsoft-com:office:word" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:wps="http://schemas.microsoft.com/office/word/2010/wordprocessingShape" xmlns:wpg="http://schemas.microsoft.com/office/word/2010/wordprocessingGroup" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" xmlns:wp14="http://schemas.microsoft.com/office/word/2010/wordprocessingDrawing" xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml" xmlns:w15="http://schemas.microsoft.com/office/word/2012/wordml" mc:Ignorable="w14 wp14 w15"><w:body><w:p><w:pPr><w:pStyle w:val="Normal"/><w:bidi w:val="0"/><w:jc w:val="left"/><w:rPr></w:rPr></w:pPr><w:r><w:rPr></w:rPr><w:t>Allusion choix</w:t></w:r></w:p><w:p><w:pPr><w:pStyle w:val="Normal"/><w:bidi w:val="0"/><w:jc w:val="left"/><w:rPr></w:rPr></w:pPr><w:r><w:rPr></w:rPr></w:r></w:p><w:sectPr><w:type w:val="nextPage"/><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:left="1134" w:right="1134" w:gutter="0" w:header="0" w:top="1134" w:footer="0" w:bottom="1134"/><w:pgNumType w:fmt="decimal"/><w:formProt w:val="false"/><w:textDirection w:val="lrTb"/></w:sectPr></w:body></w:document> |}];
  ...
  let new_xml =
    pp_xml (docx_document (Ortografe.officeopenxml `Docx ~options docx ~dst:String))
  in
...
+     Alusion chois
  ...
  let new_xml2 =
    pp_xml
      (docx_document
         (Ortografe.officeopenxml `Docx
            ~options:{ options with interleaved = false }
            docx ~dst:String))
  in
...
let%expect_test "rewrite through tag noise in docx" =
  ...
  let text =
    docx_noise
      {|<w:body><w:p><w:pPr><w:pStyle w:val="Normal"/><w:bidi w:val="0"/><w:jc w:val="left"/><w:rPr></w:rPr></w:pPr><w:r><w:rPr></w:rPr><w:t>Un deu</w:t></w:r><w:r><w:rPr></w:rPr><w:t>x</w:t></w:r><w:r><w:rPr></w:rPr><w:t xml:space="preserve"> trois quatre.</w:t></w:r></w:p><w:p><w:pPr><w:pStyle w:val="Normal"/><w:bidi w:val="0"/><w:jc w:val="left"/><w:rPr></w:rPr></w:pPr><w:r><w:rPr></w:rPr></w:r></w:p><w:sectPr><w:type w:val="nextPage"/><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:left="1134" w:right="1134" w:gutter="0" w:header="0" w:top="1134" w:footer="0" w:bottom="1134"/><w:pgNumType w:fmt="decimal"/><w:formProt w:val="false"/><w:textDirection w:val="lrTb"/><w:docGrid w:type="default" w:linePitch="100" w:charSpace="0"/></w:sectPr></w:body>|}
  in
  print_string
    (diff_strings ~context:3 (sexp_of_xmlstr text)
       (sexp_of_xmlstr
          (Ortografe.Private.convert_officeopenxml `Docx ~options text ~dst:String)));
  [%expect
    {|
                  (docx:r
  ...
  [%expect
    {|
      ...
      +             (Element (docx:t ((Text Bravil)))))))))))))))))
     |}];
  let text = docx_noise {|<w:p><w:t>e</w:t><w:t>lle elle</w:t></w:p>|} in
  print_string
    (diff_strings ~context:3 (sexp_of_xmlstr text)
       (sexp_of_xmlstr
          (Ortografe.Private.convert_officeopenxml `Docx ~options text ~dst:String)));
...
let%expect_test "odt" =
  ...
  let new_xml =
    grab_xml
      (Ortografe.opendocument
         ~options:{ options with interleaved = false }
         odt ~dst:String)
  in
  let new_xml2 = grab_xml (Ortografe.opendocument ~options odt ~dst:String) in
  print_string (diff_strings orig_xml new_xml);
  [%expect
    {|
...
let set_vm_limit =
  lazy
    (let oneGiB = Int64.of_int (1024 * 1024 * 1024) in
     ...
     Core_unix.RLimit.set vmem_limit
       { current_limit with
         cur =
...
let%expect_test "zip_bomb" =
  ...
  let urls =
    ...
    ; "https://www.bamsoftware.com/hacks/zipbomb/42.zip"
      (* version of 42.zip without the annoying password, as found at https://www.bamsoftware.com/hacks/zipbomb/ *)
  ...
  List.iter urls ~f:(fun url ->
      ...
      then
        print_s
          [%sexp
            (Result.is_ok result : bool), (url : string), (duration : Time_ns.Span.t)];
