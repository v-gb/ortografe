...
let currently_selected_rules id_prefix =
    ...
    List.filter builtin ~f:(fun rule ->
        Jv.get
          (Brrex.get_element_by_id
             (Jstr.of_string (id_prefix ^ Dict_gen_common.Dict_gen.name rule)))
          "checked"
        |> Jv.to_bool)
  in
  let custom_rule =
    Jv.get (Brrex.get_element_by_id (Jstr.of_string (id_prefix ^ "custom"))) "value"
    |> Jv.to_string
  ...
  let selection_text =
    ...
    then "rien de sélectionné"
      ...
      List.map selected_rules ~f:Dict_gen_common.Dict_gen.name |> String.concat ~sep:" "
  in
  let selection_is_nonempty = not (List.is_empty selected_rules) in
  (selected_rules, selection_text, selection_is_nonempty)

let html_fragment () =
  Dict_gen_common.Dict_gen.all_selection_html ~url_prefix:"/" ~id_prefix:"checkbox-"
    ~name_prefix:"load-" ()
...
let embedded ~lexique_url ~dict1990_url =
  ...
  Fut.ok
    { Dict_gen_common.Dict_gen.data_lexique_Lexique383_gen_tsv
...
let cached cache (type a r) key ~eq (v : a) (f : int -> a -> r) =
  ...
  | opt ->
      let i = match opt with Some (_, _, i) -> i + 1 | None -> 0 in
      let r = f i v in
...
let staged_generate cache rules_params dict_blob =
  ...
  let* dict =
    ...
    | Some (currently_selected_rules, csv1, csv2) ->
        let* next_stage =
          cached cache "next_stage" ~eq:[%equal: Jstr.t * Jstr.t] (csv1, csv2)
    ...
    | Some blob ->
        cached cache "custom_dict"
          ...
          (fun _ blob ->
            ...
            Dict_gen_common.Dict_gen.parse str ~json_of_string:Brrex.json_of_string
            |> Some __
            |> Fut.ok)
  in
  let merged_dict =
    ...
    | Some (dict, `Empty), None | Some (dict, `Nonempty), _ ->
        Dict_gen_common.Dict_gen.merge_right_biased_opt dict custom_dict
  in
  Fut.ok merged_dict

let main () =
  Brrex.main [ generate_ww_rpc ] (fun () ->
      Js_of_ocaml.Js.export "dict_gen_browser"
        (Js_of_ocaml.Js.Unsafe.inject
           (Jv.obj
              [| ("generate", generate)
               ; ( "download_from_memory"
                 , Brrex.B.(fun3' string jstr jstr unit') (fun mime filename content ->
                       Brrex.download_from_memory ~mime ~filename (`Jstr content)) )
               ; ( "staged_generate"
                 , Brrex.B.(
                     fun3' jv
